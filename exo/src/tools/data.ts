...
import { type WalletBalance } from "./helpers";
...
const getWalletBalancesTool: ToolConfig = {
  ...
  async do() {
    try {
      // Backend already returns human-readable balances (e.g. "1.5" ETH, "100" USDC)
      const wallets = await callApi<WalletBalance[]>("/wallets/balances");
      return { status: "success" as const, data: JSON.stringify(wallets), renderData: wallets };
    } catch (e) {
